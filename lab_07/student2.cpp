...
Student2::Student2(const char* name, int course, const char* group, 
    ...
    : Student1(name, course, group, recordBook, marks1) {
    for(int i = 0; i < 5; i++) {
...
void Student2::setMark2(int index, int mark) {
    if(index >= 0 && index < 5 && mark >= 1 && mark <= 5) {
        marks2[index] = mark;
    }
}
...
double groupAvg2(Student2* students[], int count, const char* group) {  // БЕЗ const
    double total = 0;
    int cnt = 0;
